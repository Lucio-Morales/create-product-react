...
const ProductCardPreview = ({ data }: { data: ProductDetails }) => {
  ...
  return (
    <div className="p-5 flex flex-col gap-10 bg-white rounded-xl shadow-xl overflow-hidden border border-gray-100 h-full">
      {/* CONTENEDOR DE IMAGENES */}
      <div className="flex h-full gap-4">
        {/* Imagen principal */}
        <div className="relative lg:-h-90 flex flex-col w-1/3 ">
          ...
          <div>
            ...
            />
            {data.stock > 0 ? (
              <span className="absolute top-12 left-2 bg-green-500 text-white text-xs font-bold px-2 py-1 rounded-full shadow-lg">
                En stock
              </span>
            ) : (
              <span className="absolute top-12 left-2 bg-gray-800 text-white text-xs font-bold px-2 py-1 rounded-full shadow-lg">
                Sin stock
              </span>
            )}
          </div>
        ...
        <div className="flex flex-col w-2/3">
          <h3 className="font-normal text-base mb-4 text-zinc-500 ">Imagenes secundarias</h3>
